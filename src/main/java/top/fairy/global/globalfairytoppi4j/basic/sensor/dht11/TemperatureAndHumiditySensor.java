...
//        dataPinData.low();
        ...
        long lowTimePulseStart = System.currentTimeMillis();
        ...
        long lowTimePulseEnd = System.currentTimeMillis();
        logger.info("thread25，实际耗时：{}", lowTimePulseEnd - lowTimePulseStart);
        dataPinData.setMode(PinMode.DIGITAL_INPUT);
        dataPinData.setPullResistance(PinPullResistance.PULL_UP);

        dataPinData.pulse(20);

        logger.info("针脚mode:{}", dataPinData.getMode());
        long lowTime83UsStart = System.nanoTime();
        ...
        if(dataPinData.getState().getValue() == 0){
            ...
            while (dataPinData.getState().getValue() == 1) {
                dataPinData.pulse(30);
            }
        }

        while (dataPinData.getState().getValue() == 0) {
            if (mark) {
                logger.info("input低电平，{}", System.nanoTime());
                mark = false;
            }

        }
        long lowTime83UsEnd = System.nanoTime();

        mark = true;
        logger.info("data应答信号低电平持续时间：{}", lowTime83UsEnd - lowTime83UsStart);

        long lowTime87UsStart = System.nanoTime();

        while (dataPinData.getState().getValue() == 1) {
            if (mark) {
                logger.info("input高电平，{}", System.currentTimeMillis());
                mark = false;
            }

        }

        long lowTime87UsEnd = System.nanoTime();

        mark = true;
        logger.info("data应答信号高电平持续时间：{}", lowTime87UsEnd - lowTime87UsStart);
